...
private nonisolated(unsafe) let importFormatter: ISO8601DateFormatter = {
    let formatter = ISO8601DateFormatter()
    formatter.formatOptions = .withInternetDateTime
    return formatter
}()
...
extension TrackPoint {
    func expectedXMLNode(withDate: Bool = false) -> GPXKit.XMLNode {
        XMLNode(
            name: GPXTags.trackPoint.rawValue,
            attributes: [
...
func assertDatesEqual(
    ...
    #expect(
        ...
        "Expected dates to be equal - expected: \(lhs), actual: \(rhs)",
...
func assertGeoCoordinateEqual(
    _ expected: any GeoCoordinate,
    _ actual: any GeoCoordinate,
    accuracy: Double = 0.0001,
    sourceLocation: SourceLocation = #_sourceLocation
) {
    #expect(
        expected.latitude.isApproximatelyEqual(to: actual.latitude, absoluteTolerance: accuracy),
        "Expected latitude: \(expected.latitude), got \(actual.latitude)",
        sourceLocation: sourceLocation
    )
    #expect(
        expected.longitude.isApproximatelyEqual(to: actual.longitude, absoluteTolerance: accuracy),
...
func assertGeoCoordinatesEqual<T: BidirectionalCollection>(
    ...
    for (lhs, rhs) in zip(expected, acutal) {
        assertGeoCoordinateEqual(lhs, rhs, accuracy: accuracy, sourceLocation: sourceLocation)
    }
}
