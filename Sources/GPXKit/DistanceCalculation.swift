//
//  Copyright © 2019 kumpelsandfriends.cc. All rights reserved.
//
...
public func distanceVincenty(_ x: (lat: Double, lon: Double),
...
func calculateSimpleDistance(from: Coordinate, to: Coordinate) -> Double {
    // https://www.movable-type.co.uk/scripts/latlong.html
    let lat1 = Measurement<UnitAngle>(value: from.latitude, unit: .degrees)
    let lat2 = Measurement<UnitAngle>(value: to.latitude, unit: .degrees)
    let lon1 = Measurement<UnitAngle>(value: from.longitude, unit: .degrees)
    let lon2 = Measurement<UnitAngle>(value: to.longitude, unit: .degrees)
    let R = 6371e3 // metres
    let φ1 = lat1.converted(to: .radians).value
    let φ2 = lat2.converted(to: .radians).value
    let Δφ = (lat2 - lat1).converted(to: .radians).value
    let Δλ = (lon2 - lon1).converted(to: .radians).value
